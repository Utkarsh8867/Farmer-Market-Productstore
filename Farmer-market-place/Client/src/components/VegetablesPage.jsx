    ...
    <div className="flex flex-col min-h-screen bg-black text-white">
      ...
      <nav className="bg-green-500 text-black p-4">
        <div className="container mx-auto flex justify-between items-center">
          <h1 className="text-xl sm:text-2xl font-bold">VegetableMart</h1>
      ...
      >
        <div className="container mx-auto">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white mb-8 text-center underline underline-offset-4 decoration-green-500">
            Vegetables
          </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {filteredVegetables.length > 0 ? (
              filteredVegetables.map((vegetable) => (
                ...
                >
                  <img
                    src={vegetable.image.startsWith("http")
                        ? vegetable.image
                        : `https://server-fmp.onrender.com${vegetable.image}`}
                    alt={vegetable.name}
                    className="w-full h-40 object-cover rounded-md"
                  />
                  <div className="mt-3">
                    <h3 className="text-md sm:text-lg font-bold text-white truncate">{vegetable.name}</h3>
                    <div className="flex justify-between items-center text-sm">
                      <p className="text-green-100 font-bold">
                        ₹{vegetable.discountPrice || vegetable.originalPrice}
                        <span className="line-through text-gray-400">₹{vegetable.originalPrice}</span>
                      </p>
                      <span className="text-gray-300 font-medium">{vegetable.quantity}kg</span>
                    </div>
              ...
              <p className="text-white text-center">No vegetables available.</p>
            )}
          </div>
        </div>
      </section>
